...
//  evaApp
//
//  Created by Alex Kharchenko on 18.12.2020.
//  Copyright © 2020 Александр Харченко. All rights reserved.
//
...
fileprivate extension UIImage {
    ...
    func decodedImage() -> UIImage {
            ...
            let context = CGContext(data: nil, width: Int(size.width / 2), height: Int(size.height / 2), bitsPerComponent: 2, bytesPerRow: cgImage.bytesPerRow, space: colorSpace, bitmapInfo: CGImageAlphaInfo.noneSkipFirst.rawValue)
...
public final class ImageLoader {
    ...
    public func loadImage(from url: URL) -> Observable<UIImage?>  {
        if let image = cache[url] {
            return Observable.just(image).asObservable()
        }
        ...
        return URLSession.shared.rx.data(request: request)
            ...
            .catchError { error in return Observable.never() }
